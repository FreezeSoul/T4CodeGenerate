// -----------------------------------------------------------------------
// <copyright file="PostgreSqlSchemaReader.cs" company="">
...
namespace CodeGen.Readers
{
    ...
    public class PostgreSqlSchemaReader : SchemaReader
    {
            ...
            using (var sqlCommand = new NpgsqlCommand(TableSql, _connection))
            {
                using (var reader = sqlCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        tbl.Comment = reader["table_comment"].ToString();
            ...
            using (var sqlCommand = new NpgsqlCommand(ColumnSql, _connection))
            {
                ...
                using (var reader = sqlCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        col.Comment = reader["column_comment"].ToString();
        ...
        private const string ColumnSql = @"
			SELECT column_name, is_nullable, udt_name, column_default, column_comment
